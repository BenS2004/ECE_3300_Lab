...
module barrel_shifter16(
    ...
    wire [15:0] level0[0:15], rot_val[0:15], logr_val[0:15], logl_val[0:15];
    wire [15:0] level1[0:7];
    wire [15:0] level2[0:3];
    wire [15:0] level3[0:1];
    
    wire [3:0] select;
    assign select = (shamt & {16{dir}}) | ({16{(~dir)}} & ((16 - shamt)%16));
    ...
    generate
        ...
        for(i = 1; i < 16; i = i + 1)begin    
            assign rot_val[i] = ({16{rot}} & ({data_in[0+i-1:0], data_in[15:i]}));    
            assign logr_val[i] = ({16{~rot}}&{16{dir}} & (16'hffff >> i) & {data_in[0+i-1:0], data_in[15:i]});
            assign logl_val[i] = ({16{~rot}}&{16{~dir}} & (16'hffff << (16 - i)) & {data_in[0+i-1:0], data_in[15:i]});
            assign level0[i] = rot_val[i] | logr_val[i] | logl_val[i];
